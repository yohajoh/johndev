...
export const CustomCursor = () => {
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      // Add many trail points (dots) with random colors and dispersion
      const newTrailPoints: TrailPoint[] = Array.from({ length: 8 }).map(
        (_, i) => ({
          id: Date.now() + i,
          x: e.clientX + (Math.random() - 0.5) * 8, // Increased offset for more dispersion
          y: e.clientY + (Math.random() - 0.5) * 8,
          size: isHovering ? 14 + Math.random() * 6 : 12 + Math.random() * 6, // Bigger dots
          opacity: 1,
          color:
          ...
          timestamp: Date.now(),
          velocity: {
            x: (Math.random() - 0.5) * 6, // Increased random velocity for more dispersion
            y: (Math.random() - 0.5) * 6,
          },
        })
      );
      setTrail((prev) => [...prev.slice(-100), ...newTrailPoints]); // More trail points (up to 100)
      ...
      if (velocity > 10) {
        const newParticles = PARTICLE_CONFIGS.map((config) => ({
          ...
          x: e.clientX,
          y: e.clientY,
          size: config.size + 4, // Bigger particles
          color: colors[config.colorKey as keyof typeof colors],
          lifetime: 1000,
          velocity: {
            x: config.velocity.x + (Math.random() - 0.5) * 4, // More random velocity
            y: config.velocity.y + (Math.random() - 0.5) * 4,
          },
        }));
  ...
  // Update particles and trail for smooth animation
  useEffect(() => {
    if (isMobile) return;
    const interval = setInterval(() => {
      ...
      // Update trail with dispersion and smoother decay
      setTrail(
        (prev) =>
          prev
            .map((p) => ({
              ...
              x: p.x + p.velocity.x,
              y: p.y + p.velocity.y,
              opacity: p.opacity * 0.95, // Slower fade for smoother animation
              size: p.size * 0.98, // Slower shrink
              velocity: {
                x: p.velocity.x * 0.92, // Smoother dampening
                y: p.velocity.y * 0.92,
              },
            }))
            .filter((p) => p.opacity > 0.02) // Lower threshold for longer trails
      );
    }, 8); // Faster update interval for smoother animation (~120fps)
  ...
  return (
    <>
      {/* Trail Effect - Many dots, dispersed, colorful, bigger, smoother */}
      <AnimatePresence>
        {isVisible &&
          trail.map((point) => (
            <motion.div
        ...
        {particles.map((particle) => (
          <motion.div
            ...
            style={{
              ...
              boxShadow: `0 0 12px ${particle.color}80`,
            }}
